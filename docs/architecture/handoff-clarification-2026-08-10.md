...
지금은 되묻지 않는다. §4 를 보라 — 현재 기본값 `file_search` 는 45문항 **전부**에서 무언가를
주장했고 그중 14.2%가 규정 원문에 근거가 없었다. 모를 때 모른다고 말하지 않는 것이 이 제품의
가장 큰 결함이다.

### 끝났을 때 있어야 할 것

1. **결정적 재질문 트리거** — 검색 점수 임계값 기반. `lexical` 이 반환하는
   `Retrieved.top_score` / `ranked_units` 를 읽어 「근거 부족」을 판정한다 (§3)
2. **임계값의 근거** — 45문항 코퍼스로 임계값을 스윕해서, 어디를 자르면 무주장 6건을
   잡으면서 정상 답변을 몇 건 희생하는지 표로 낸다. 임의로 고른 숫자는 받아들이지 않는다
3. **소규모 실측** — 트리거가 걸린 질문에 되물은 뒤, 받은 답으로 다시 답했을 때 지어냄율이
   실제로 내려가는지 잰다. 전체 45문항일 필요 없다. 트리거가 걸리는 문항만으로 충분하다
4. **판단 기록** — `docs/architecture/` 에 남긴다. 대화 요약이 아니라 이 문서처럼
   검증된 사실 + 확인 명령의 목록으로 쓴다

### 제약 — 이유와 함께

- **LLM 에게 "되물을까 말까"를 묻지 마라.** PR #51 이 그 방식으로 실패했다(60문항 중 30건
  경로 불일치, 클로즈). 판정은 검색 점수라는 결정적 신호에서 나와야 한다. LLM 은
  「무엇을 되물을지」 문장을 만드는 데만 쓴다 — 그건 이미 §2-② 가 한다
- **재질문 레이어를 새로 만들지 마라.** §2-② 의 것들이 이미 있다. 없는 것은 신호 하나뿐이다
- **`file_search` 위에서는 만들 수 없다.** §3 참조. `lexical` 이어야 한다
- **측정 지표는 재질문 발생률이 아니다.** §8 참조. 되묻기만 늘고 답이 그대로면 실패다
- **7개 종착 경로를 확인하라.** §5-① 참조. 하나만 고치면 나머지 6개로 새어나간다
- **라이브 DB 에 쓰지 마라.** 읽기만 하고, 쓰기가 필요하면 사용자에게 먼저 물어라.
  `backend/.env` 에 Neon 자격증명이 있다 — 커밋 금지
- **측정 하네스 재실행 시 `--retry-failed`.** §7 참조

### 열려 있는 것 — 다음 세션이 정한다

- 임계값을 페이지 공간 점수로 잡을지 유닛 공간 점수로 잡을지 (RRF 가 두 번 돈다, §3)
- 단일 임계값인지 2단(항상 묻기 / 가끔 묻기 / 조용히 답하기)인지
  — "Clarify Once, Learn the Default" 는 0.60 / 0.85 2단을 권한다
- 확정된 기본값을 `ops_facts` 에 넣을지 (구조가 같다. 지금 0행)
