...
────────────────────────────────────────────────────────────────────────────
⚠ 운용 정책 — FAQ 는 **차단·회피 전용**이다 (2026-08-11 확정)
────────────────────────────────────────────────────────────────────────────
**FAQ 에 「정답」을 등록하지 마라.** 공격적·악의적 질문을 막는 용도로만 쓴다.

왜: FAQ 가 히트하면 `chat_service` 는 **RAG·운영사실·히스토리를 통째로 건너뛰고**
지정답변을 그대로 반환한다(`source=faq_override`). 그래서 **살짝 다른 질문이
near-miss 로 걸리면 「애매하게 틀린 답」이 근거 없이 나간다.** 실제로 겪은 사고다 —
"축복을 왜 받아야 해요?" 가 가정회비 FAQ 에 오매칭됐고, 가정폭력 질문이 성격차이
FAQ 에 0.93 으로 걸렸다. 임계값을 올려도 양성·음성 유사도 분포가 겹쳐(−0.078)
전역 분리값이 없다.

그래서:
  ✅ 허용 — 공격·악의적 질문 차단, 회피·확인유도 문구
           (near-miss 로 걸려도 「과회피」일 뿐 틀린 답이 아니다)
  ❌ 금지 — 사실 답변 치환. 하위 케이스가 많은 토픽(세대 규정·시점 분기 등)은 특히
           위험하다. 살짝 다른 질문에 발동하면 그대로 오답이다

**「규정집에 답이 없어서 못 답한다」는 FAQ 로 처리하지 마라.** 그건 `ops_facts` 의
일이다 — `ops_facts` 는 프롬프트에 얹힐 뿐 **RAG 를 건너뛰지 않아서** 이 사고가
구조적으로 안 난다(`chat_service.py` 의 overlay 지점 참조).

경위: `docs/architecture/faq-usage-policy.md`
────────────────────────────────────────────────────────────────────────────
"""
