# 위기 신호 감지·환각 번호 필터·차단 고정문 — 위기 대응의 코드 레벨 안전장치 (P0)
"""
위기·민감 대응의 코드 레벨 안전장치.

대응 지시(확인 단계·생성형·동선)는 2026-06-12 결정으로 봇 시스템 프롬프트 본문(위기 섹션·Edit A)으로
이관됐고, 이 모듈은 감지·번호 필터·차단 폴백만 담당한다.

설계 원칙 (exports/crisis_rule_2026-06-12 검증 + 2026-06-12 사용자 확정):
- 키워드는 "후보 신호"일 뿐 — 자동 차단/고정문 전환에 쓰지 않는다.
  FAQ 스킵·번호 필터(strip)·로깅 트리거로만 쓴다.
- 급성 위기는 검증된 고정 핫라인(109·1577-0199·112·119 등)을 노출하되, 봇이 지어낸
  환각 번호는 strip_phone_sentences 가 코드 단에서 제거한다 (일본 사례: 잘못된 핫라인 안내가 더 위험).
