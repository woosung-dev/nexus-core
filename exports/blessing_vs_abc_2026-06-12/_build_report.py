# 블레싱 나 vs A/B/C 비교 보고서 — 페르소나 에이전트 + codex 이중 평가를 단일 HTML로
import html
import json
...
OUT = Path("/Users/woosung/Downloads") / f"블레싱_나_vs_ABC_레드팀비교_{date.today()}.html"
USERS = ["조화연", "신은비", "김소영"]
...
for u in USERS:
    ...
    for it in items:
        ...
        <div class="qbody">
          ...
          <div class="ans bless"><div class="al">★ 블레싱 나 (신규) <span class="cite">인용: {esc(cite_str)}</span></div><div class="at">{esc(it.get('blessing_answer'))}</div></div>
...
<title>블레싱 나 vs A/B/C — 레드팀 비교 ({date.today()})</title>
<style>
...
<header>
  ...
  <div class="meta">조화연·신은비·김소영 2주차 레드팀 질문 각 30문항(총 90) · 신규 봇 "블레싱 나"(bot id=3, gemini-3.1-flash-lite) 재질의 · 페르소나 에이전트 + codex 이중 평가 · {date.today()}</div>
</header>
<div class="cards">
...
{"".join(sections)}
<div class="note"><b>방법</b> · A/B/C = 2주차 레드팀에서 테스터가 본 3개 봇(통합/원리/정밀) 답변(고정). 블레싱 나 = 신규 봇(bot id=3)을 동일 질문에 운영동일 파라미터(generate_with_rag, max_tokens=2048)로 재질의한 새 답변. 평가는 (1) 각 테스터의 페르소나 레드팀 에이전트(Claude)와 (2) codex(독립 LLM)가 동일 루브릭·도메인 정답기준으로 독립 수행. blessing_vs_tester 는 테스터가 당시 고른 답변 대비 블레싱의 우열.</div>
</div>
<script>
