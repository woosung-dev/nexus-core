...
- 평가 데이터: dataset_${user}_v2.json (items 배열의 각 항목에 q, ansA_통합, ansB_원리, ansC_정밀, tester_choice, tester_win, tester_feedback, blessing_answer 가 있다).
...
최종 응답은 eval_schema.json 스키마를 따르는 JSON 객체 하나만 출력하라. evaluator 는 'codex', user 는 '${user}' 로 채워라."
  ...
  codex exec \
    ...
    -o "$DIR/codex_eval_v2_${user}.json" \
    "$prompt" > "$DIR/codex_v2_${user}.log" 2>&1
