# 블렌드 2봇(정밀기반·통합기반) + 통합 raw 베이스라인을 운영동일 모델로 프로브(스테이징 id3 RAG)
# 사용: cd backend && set -a; source .env; set +a; uv run python ../exports/_probe_run_blend.py
import asyncio
import json
import sys
...
OUT = Path("/Users/woosung/project/agy-project/nexus-core/exports/round3_redteam/04_평가·프로브/probe_answers_blend.json")
MODEL = "gemini-3.1-flash-lite"
STAGING_BOT = 3

CANDIDATES = {
    "정밀기반_블렌드": RAGDIR / "final_정밀기반.md",
    "통합기반_블렌드": RAGDIR / "final_통합기반.md",
    "통합_raw_codex2": RAGDIR / "_base_통합_id3_codex2.md",
}
