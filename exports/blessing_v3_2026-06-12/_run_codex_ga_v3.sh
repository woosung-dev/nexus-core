...
- 평가 데이터: dataset_가_${user}_v3.json (blessing_answer 는 블레싱 가 v3 답변).
모든 질문 평가. eval_schema.json 스키마 JSON 하나만 출력. evaluator='codex', user='${user}'."
  echo "[codex 가/$user]"; codex exec -C "$DIR" -s read-only --skip-git-repo-check \
    --output-schema "$DIR/eval_schema.json" -o "$DIR/codex_가_${user}_v3.json" "$p" > "$DIR/codex_가_${user}_v3.log" 2>&1; }
