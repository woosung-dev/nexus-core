...
JOBS = [("나", ["조화연","신은비","김소영"], DIR/"answers_나_v5.json"),
        ("가", ["미야자키시호","김소영","조화연"], DIR/"answers_가_v5.json")]
...
for bot, users, ansf in JOBS:
    ...
    for u in users:
        ...
        json.dump({"user":u,"agent":AGENT[u],"count":len(items),"items":items},
                  open(DIR/f"dataset_{bot}_{u}_v5.json","w"), ensure_ascii=False, indent=1)
        errs=sum(1 for it in items if (it["blessing_answer"] or "").startswith("[ERROR]"))
        print(f"  {bot} {u}: dataset_{bot}_{u}_v5.json ({len(items)}, ERROR {errs})")
print("v3 병합 완료")
