# 블레싱 가 원본 vs v3 vs A/B/C 통합 — 따뜻한 실무 안내자 프롬프트 개선 검증
import html, json
...
OUT = Path("/Users/woosung/Downloads") / f"블레싱_가_원본v3_vs_ABC_{date.today()}.html"
...
ds = {
  "g": {u: json.load(open(GA/f"dataset_{u}.json")) for u in USERS},
  "v3": {u: json.load(open(V3/f"dataset_가_{u}_v3.json")) for u in USERS},
}
def aidx(p):
    raw = load(p) or []
...
def cidx(folder, prefix, suffix=""):
    ...
    for u in USERS:
        ...
        if ce: by[u] = {r["qid"]: r for r in ce.get("results", [])}
    return (by,)
A = {"g": aidx(GA/"agent_eval.json"), "v3": aidx(V3/"agent_eval_가_v3.json")}
C = {"g": cidx(GA, "codex_eval_"), "v3": cidx(V3, "codex_가_", "_v3")}
VERS = ["g", "v3"]; VN = {"g": "원본 (따뜻한 실무)", "v3": "v3 (구조수정)"}
...
sections, ucards = [], []
for u in USERS:
    items={it["qid"]:it for it in ds["g"][u]["items"]}
    v3it={it["qid"]:it for it in ds["v3"][u]["items"]}
    def ucount(ver,kind):
        src=A[ver][0] if kind=="A" else C[ver][0]; rs=src.get(u,{}).values()
    ...
    for kind,kn in [("A","페르소나"),("C","codex")]:
        for ver in VERS:
            b,w,l=ucount(ver,kind); rows_agg+=f'<tr><td>{kn} {VN[ver].split()[0]}</td><td>{b}</td><td>{w}</td><td>{l}</td></tr>'
    ...
    rows=[]
    for qid,itg in items.items():
        it3=v3it.get(qid,{}); tester=WL.get(itg.get("tester_win"),itg.get("tester_choice"))
        def avg(ver):
            xs=[bscore(A[ver][0].get(u,{}).get(qid)),bscore(C[ver][0].get(u,{}).get(qid))]; xs=[x for x in xs if isinstance(x,int)]
            return sum(xs)/len(xs) if xs else None
        ag,a3=avg("g"),avg("v3"); d=(a3 or 0)-(ag or 0)
        cls="dup" if d>0.4 else ("ddn" if d<-0.4 else "dmid"); arrow="▲" if d>0.4 else ("▼" if d<-0.4 else "＝")
        trend = (f"{ag:.1f}→{a3:.1f}" if ag is not None and a3 is not None else "-")
        ...
        <summary><span class="qid">{esc(qid)}</span><span class="qt">{esc(itg.get('qtype'))}</span>
        <span class="qq">{esc(itg['q'][:58])}</span>
        <span class="qmeta">테스터→<b>{esc(tester)}</b> <span class="delta {cls}">{arrow} {trend}</span></span></summary>
        <div class="qbody"><div class="qfull"><b>질문</b> {esc(itg['q'])}</div>
          <div class="ans"><div class="al">A · 통합</div><div class="at">{esc(itg.get('ansA_통합'))}</div></div>
          <div class="ans"><div class="al">B · 원리</div><div class="at">{esc(itg.get('ansB_원리'))}</div></div>
          <div class="ans"><div class="al">C · 정밀</div><div class="at">{esc(itg.get('ansC_정밀'))}</div></div>
          {ansrow(itg,"블레싱 가 원본","blv1")}{ansrow(it3,"★ 블레싱 가 v3 (신규)","blv3")}
          <div class="tf"><b>테스터({esc(u)}) 원선택</b> {esc(tester)} · <b>피드백</b> {esc(itg.get('tester_feedback'))}</div>
          {ev_rows}</div></details>""")
...
def deltaline():
    ...
    return f"원본→v3: 페르소나 best {sign(agg['v3']['A'][0]-agg['g']['A'][0])} · codex best {sign(agg['v3']['C'][0]-agg['g']['C'][0])}"

HTML=f"""<!DOCTYPE html><html lang="ko"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
<title>블레싱 가 원본·v3 vs A/B/C — {date.today()}</title><style>
:root{{--ink:#1A2233;--sub:#5A6678;--line:#E5E9F0;--bg:#F6F8FB;--card:#fff;--accent:#9333EA;--v3:#0D9488;}}
...
<header><div class="eyebrow">REDTEAM · 블레싱 가(따뜻한 실무) 원본→v3 개선</div><h1>블레싱 가 원본·v3 vs A·B·C</h1>
<div class="meta">미야자키시호·김소영·조화연 각 30문항(90) · 같은 봇(id=5) 프롬프트 원본→v3 재질의 · 페르소나+codex 이중 평가 · {date.today()}</div></header>
<div class="deltabar">📈 {deltaline()} · 문항 화살표는 두 평가자 평균 블레싱 점수 원본→v3 변화</div>
<div class="vcards">{vcard("g")}{vcard("v3")}</div>
<h2>사용자별 원본 vs v3 집계</h2><div class="ucards">{"".join(ucards)}</div>
<div class="filter"><button class="on" data-f="all">전체</button><button data-f="미야자키시호">미야자키시호</button><button data-f="김소영">김소영</button><button data-f="조화연">조화연</button></div>
{"".join(sections)}
<div class="note"><b>방법</b> · A/B/C=2주차 3봇 고정. 블레싱 가 원본·v3=같은 봇(id=5) 프롬프트만 바꿔 동일 90문항 재질의. v3=원본에 구조수정 5종+성물(성초) 패치 적용. 평가는 페르소나+codex 독립. 화살표=두 평가자 평균 블레싱 점수 원본→v3.</div>
...
print(f"저장: {OUT}")
for ver in VERS: print(f"  {VN[ver]}: 페르소나 best {agg[ver]['A'][0]}/codex {agg[ver]['C'][0]}")
print(" ", deltaline())
