...
- 평가 데이터: dataset_나_${user}_v5.json (blessing_answer 는 블레싱 나 v5 답변).
모든 질문 평가. eval_schema.json 스키마 JSON 하나만 출력. evaluator='codex', user='${user}'."
  echo "[codex 나/$user]"; codex exec -C "$DIR" -s read-only --skip-git-repo-check \
    --output-schema "$DIR/eval_schema.json" -o "$DIR/codex_나_${user}_v5.json" "$p" > "$DIR/codex_나_${user}_v5.log" 2>&1; }
